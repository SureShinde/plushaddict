...
class HighlightPlugin
{
    ...
    public function highlight($html, $query)
    {
        ...
        foreach ($matches[0] as $key => $match) {
            ...
            $strippedMatchWithHolder = [];

            foreach ($queryWithHolders as $key => $value) {
                $strippedMatchWithHolder = array_replace(
                    $strippedMatchWithHolder,
                    explode(' ', str_ireplace($key, $value, $strippedMatch))
                );
            }

            $strippedMatchWithHolder = implode(' ', $strippedMatchWithHolder);
