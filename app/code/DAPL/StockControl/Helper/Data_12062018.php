...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
	...
	private function recalculateMetreStock($cut_product, $metre_product, $purchased_qty){
		...
		//$this->logger->addInfo("old_stock_metre: ".$old_stock_metre);
		//$this->logger->addInfo("purchased_qty: ".$purchased_qty);
		//$this->logger->addInfo("quantity_of_cut: ".$quantity_of_cut);
		//$this->logger->addInfo("length_of_cut: ".$length_of_cut);
		//$this->logger->addInfo("part cal: ".$stock_cut_product % $quantity_of_cut);

		//$new_stock_metre = $old_stock_metre -(ceil(($stock_cut_product % $quantity_of_cut) / $quantity_of_cut)* $length_of_cut );
		$new_stock_metre = $old_stock_metre -round(($purchased_qty/4),1);
		//$this->logger->addInfo("new_stock_metre: ".$new_stock_metre);
	...
	public function processOrderForStockControl($orderId){
			...
			foreach ($metre_product_ids as $metre_product_id)
			{
				...
				$this->processSavedProduct($metre_product);
			}
	}
}